...
let a = func1<number>([4, 2]); // <number>는 MyType으로 되서 함수 어디서든 사용 가능하다
...
function homework1<HType extends string | string[]>(a: HType): number {
  return a.length;
}
console.log("homework1_1", homework1<string>("hello"));
console.log("homework1_2", homework1<string[]>(["kim", "park"]));
// (동작 예시)
...
function homework2<H2Type>(obj: string): H2Type {
  return JSON.parse(obj);
  // H2Type을 안쓰고 JSON.parse(obj) as animal로 해도 된다.
}

console.log("homework2", homework2<Animal>(data));
// (동작 예시)
